...
// XXX: finish and move elsewhere
/*
#if (CTRLR_TYPE == CTRLR_RGB)
typedef struct {uint8_t r, g, b;} rgb_t;
typedef struct {uint8_t h, s, v;} hsv_t;

static hsv_t hsvled[4];

rgb_t rgb2hsv(hsv_t hsv) {

    rgb_t rtn;
    uint16_t c = hsv.v * hsv.s;
    uint16_t x;

    if ( hsv.h <= 42 ) {
        x = (c*hsv.h)/42;
        rtn.r = c; rtn.g = x; rtn.b = 0;
    } else if ( hsv.h < 42*2 ) {
        x = (c*(42*2-hsv.h))/42;
        rtn.r = x; rtn.g = c; rtn.b = 0;
    } else if ( hsv.h <= 42*3 ) {
        x = (c*(42*2-hsv.h))/42;
        rtn.r = 0; rtn.g = c; rtn.b = x;
    } else if ( hsv.h < 42*4 ) {
        x = (c*(42*4-hsv.h))/42;
        rtn.r = 0; rtn.g = x; rtn.b = c;
    } else if ( hsv.h <= 42*5 ) {
        x = (c*(42*4-hsv.h))/42;
        rtn.r = x; rtn.g = 0; rtn.b = c;
    } else {
        x = (c*(42*6-hsv.h))/42;
        rtn.r = c; rtn.g = 0; rtn.b = x;
    }
